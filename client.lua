...
robberyActive = false
RegisterNetEvent('DeadBankRobbery:IsActive:Return')
AddEventHandler('DeadBankRobbery:IsActive:Return', function(bool)
	robberyActive = bool
end)
			...
			if (config.enableBanks == true) then
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, bankcoords.x, bankcoords.y, bankcoords.z, true) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E key
						TriggerServerEvent('DeadBankRobbery:SetActive', true)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		TriggerServerEvent('DeadBankRobbery:IsActive')
	end
end)
